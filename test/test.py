...
def timeit(stmt, setup=""):
    t = Timer(stmt, setup, globals=globals())
...
t = timeit("ce.matrix.random((1000,1000))")
print(f"random: {t:1.3g} mus/iteration (Cython version)")

t = timeit("rng.random((1000,1000))", "rng=np.random.default_rng()")
print(f"random: {t:1.3g} mus/iteration (Numpy version)")
